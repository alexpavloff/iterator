class ChannelMember {
    constructor(myid, joined, left, names) {
        this.id = myid;
        this.joined = joined;
        this.left = left;
        this.names = names;
...
class ChannelMembersHistory {
    constructor(data) {
        this.data = data;
    }
    createInterator(type = "", reversed = false) {
        return new ChannelMembersHistoryIterator(this.data, type, reversed);
    }
}
class ChannelMembersHistoryIterator {
    constructor(data, type = '', reversed = false) {
        this.data = data;
        this.index = 0;
...
let A = new ChannelMembersHistory([
    ...
    new ChannelMember(333993337, [1610906178, 1628015226, 1629883767], [1628013378, 1629394092], ["Владислав"]),
    new ChannelMember(320625292, [1622717396, 1627556180], [1617978480, 1622791226, 1629872353], ["untrpalad77", "untr svtnsny", "ursa77", "unter_svetonosny"]),
    new ChannelMember(614592605, [1622107732, 1623266914, 1629578102, 1631199711], [1622976313, 1626926515, 1629628658], ["kedrokarp", "Vladimir Nosikov"])
]);
